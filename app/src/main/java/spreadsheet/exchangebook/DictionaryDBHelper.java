...
class DictionaryDBHelper extends SQLiteOpenHelper {
  ...
  //  private static final String DATABASE_TO_CURRENCY = "to_currency";
   ...
   //                 res.getString(res.getColumnIndex(DATABASE_TO_CURRENCY)),
    ...
    public long insertExcengeRecord(int from_cents, String from_currency, int to_cents, String created, String comment, String hash_value) {
   ...
   //     contentValues.put(DATABASE_TO_CURRENCY, to_currency);
    ...
    public long insertExchangeRecordWithHash(int from_cents, String from_currency, int to_cents, String comment) {
        String created = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").format(new Date());


        String hash_value = SHA1(this.lastHash() + created + ":" + Integer.toString(from_cents) + ">" + from_currency + "<" + Integer.toString(to_cents) +  ":" + comment);
        return this.insertExcengeRecord(from_cents, from_currency, to_cents, created, comment, hash_value);
    }
